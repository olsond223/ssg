...
def generate_page(from_path, template_path, dest_path, basepath):
    ...
    dest_contents = template_contents.replace("{{ Title }}", title).replace("{{ Content }}", content).replace('href="/', f'href="{basepath}').replace('src="/', f'src="{basepath}')
...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path, basepath):
    for entry in os.listdir(dir_path_content):
        ...
        if os.path.isdir(filepath):
            generate_pages_recursive(filepath, template_path, dest_dir_path, basepath)
